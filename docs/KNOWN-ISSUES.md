...
- `tests/e2e/exported-fixtures.spec.js` adds standalone (non-builder-embedded) keyboard-operability coverage for 7 components — Accordion, Flip Cards, Tabs, Multiple Choice, Hotspots, Vertical Timeline, Audio Player — closing the pre-existing gap where Hotspots and Audio/Video had no dedicated e2e keyboard test at all (`tests/e2e/interactions.spec.js` covers the other 5 of these 7). Interactive Video also has extensive dedicated e2e keyboard coverage (arrow-key answer selection, Tab traversal, marker-nav operability), but in its own separate file (`tests/e2e/interactive-video.spec.js`), not this shared fixture list. The remaining 13 of the 21 components still rely only on the unit-level coverage above for keyboard behavior.
