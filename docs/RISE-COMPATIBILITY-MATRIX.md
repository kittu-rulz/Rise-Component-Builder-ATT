...
| Surface | Tier | Why |
| --- | --- | --- |
| Rise authoring preview (the "Preview" button while editing a course) | **Preview** | No access to a live Rise course as part of this project's automation. Authoring-mode preview panes in some course-authoring tools sandbox embedded content more strictly than the tool's final published output — untested here either way. |
| Rise share-link preview (a shareable pre-publish preview link) | **Preview** | Same reasoning as authoring preview; a different rendering context Rise may or may not treat identically to the published course. |
