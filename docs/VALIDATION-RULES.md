...
| `general-completion-iframe-format` | Warning | Completion tracking is on. Option A (Iframe Snippet) nests the component one level too deep inside Rise's own sandbox for Rise's Continue-block gating to detect the completion signal — Option B (HTML Block Fragment) does not have this problem (see `docs/COMPATIBILITY-RESULTS.md`). Fires unconditionally whenever completion tracking is on, since Preflight runs before the author picks an export format. |
