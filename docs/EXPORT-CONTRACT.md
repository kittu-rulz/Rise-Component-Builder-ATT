...
- **CSS isolation, by format:**
  ...
  - The **HTML fragment** format (for pasting directly into a host page's own DOM) has no such boundary. Class names are not automatically rewritten/prefixed for this format — components use reasonably specific class names (e.g. `.accordion-group`, `.hotspot-tooltip-title`) but a host page's own styles could still coincidentally collide on a generic name. This is a deliberate, documented trade-off (not a gap the fragment format claims to close): the export modal's own copy explains it directly to the author rather than hiding it — the iframe embed is described as "the safest default if you're unsure which option to pick" because of this isolation guarantee, while the fragment format is positioned for components whose content dynamically resizes (accordions, timelines, knowledge checks revealing feedback), since it isn't boxed into the iframe's fixed-height container. Neither format carries a "Recommended" badge in the UI — that badge only appears on the Rise Project ZIP option, and only when the component has uploaded media (`updateRiseZipRecommendedBadge()`, `app.js`), which is unrelated to CSS isolation. The id/JS-global guarantees above hold for the fragment format regardless.
