...
- **CSS isolation, by format:**
  ...
  - The **HTML fragment** format (for pasting directly into a host page's own DOM) has no such boundary. Class names are not automatically rewritten/prefixed for this format — components use reasonably specific class names (e.g. `.accordion-group`, `.hotspot-tooltip-title`) but a host page's own styles could still coincidentally collide on a generic name. This is a deliberate, documented trade-off (not a gap the fragment format claims to close): the export modal's own copy explains it directly to the author rather than hiding it — the iframe embed is positioned for "reliable isolation," while the fragment format is positioned for components whose content should "expand naturally" (accordions, timelines, knowledge checks revealing feedback), since it isn't boxed into the iframe's fixed-height container (`js/compatibility.js`). The export modal expresses its recommendation structurally rather than with a badge: the HTML fragment format is the always-visible primary option, while the iframe embed and Web Package ZIP sit under a collapsed "Advanced export options" disclosure — except when the component has uploaded media too large to inline, in which case the primary section is replaced by a notice pointing the author at Web Package ZIP instead (`updatePrimaryExportSection()`, `app.js`). The id/JS-global guarantees above hold for the fragment format regardless.
...
| Web Package ZIP         | `buildRiseProjectZip()` (`js/export.js`), packaged by `js/zip.js`              | A real ZIP: the compiled document at `index.html` (ZIP root, no wrapper directory), every referenced upload under `assets/<sanitized-filename>`, and `assets/manifest.json` — see `docs/MEDIA-ASSET-PIPELINE.md` |
...
This claim and the per-browser/per-condition breakdown (keyboard-only operation, restricted networks, offline fonts) are formalized in **`docs/RISE-COMPATIBILITY-MATRIX.md`**, which classifies every export workflow and target surface into one of four tiers (Confirmed / Preview / Fallback / Unsupported) and cites the exact evidence behind each — including its "Scope" section, which records that Moodle compatibility and SCORM packaging are explicitly out of scope for this project, not pending gaps. The same tiers drive the export compatibility report shown in the Export modal (`js/compatibility.js`, rendered by `renderExportCompatibilityReport()` in `app.js`) — the in-app UI and this document are required to never disagree. The manual test procedure lives in `docs/RISE-TEST-CHECKLIST.md`; actual results (not just claims) are logged in `docs/COMPATIBILITY-RESULTS.md`. (`docs/MOODLE-SCORM-TEST-CHECKLIST.md` is kept for reference only.)
